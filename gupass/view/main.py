...
class MainWindow(Tk.Frame):
    ...
    def __init__(self, controller, master=None, width=730, height=800):
        ...
        self.master.title('gupass')
        self.master.resizable(width=True, height=True)
        self.master.geometry('{}x{}'.format(width, height))
        
        # This is the icon in Windows and X Windows
        icon_image = Tk.PhotoImage(file=os.path.join(icons.icon_dir, 'gupass.gif'))
        ...
        if sys.platform == 'darwin':
            ...
            app_menu.add_command(label='About gupass', command=do_about_dialog)
        ...
        helpmenu.add_command(label='About gupass', command=do_about_dialog)
    ...
    def on_process(self, type_):
        ...
        if opt_file_path:
            ...
            if type_ == 'full':
                ...
                lines = ['# Rules Generated by', '# gupass', '#',
                         '# Rule chain', '# ---------------------']
                if len(self.nodes) > 1:
    ...
    def on_save(self):
        ...
        '''
        path = tkinter.filedialog.asksaveasfile(parent=self.master, filetypes=[("gupass chain files", "*.gupass")])
        if path:
